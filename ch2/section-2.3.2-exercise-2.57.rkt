  ...
  (cond ((number? exp) 0)
        ...
        ((exponentiation? exp)
         (make-product
          ...
          (deriv (base exp) wrt)))
        (else
         (error "`deriv` has encountered an unknown expression type:" exp))))
...
(define (make-sum addend . augends)
  (cond ((null? augends) addend)
        ((=number? addend 0)
         (apply make-sum augends))
        (else
         (let ((sum-parts (foldl (lambda (item sofar)
                                   (if (number? item)
                                       (cons (+ item (car sofar))
                                             (cdr sofar))
                                       (cons (car sofar)
                                             (append (cdr sofar)
                                                     (list item)))))
                                 (list 0)
                                 (append (list addend) (make-sum augends)))))
           (cond ((= (length sum-parts) 1)
                  (car sum-parts))
                 (else (append (list '+) (if (=number? (car sum-parts) 0)
                                             (cdr sum-parts)
                                             sum-parts))))))))

(define (make-product multiplicand . multipliers)
  (cond ((null? multipliers) multiplicand)
        ((=number? multiplicand 1)
         (apply make-product multipliers))
        (else
         (let ((product-parts (foldl (lambda (item sofar)
                                       (if (number? item)
                                           (cons (* item (car sofar))
                                                 (cdr sofar))
                                           (cons (car sofar)
                                                 (append (cdr sofar)
                                                         (list item)))))
                                     (list 1)
                                     (append (list multiplicand) (make-product multipliers)))))
           (cond ((= (length product-parts) 1)
                  (car product-parts))
                 ((=number? (car product-parts) 0) 0)
                 ((=number? (car product-parts) 1)
                  (if (= (length (cdr product-parts)) 1)
                      (cadr product-parts)
                      (append (list '*) (cdr product-parts))))
                 (else
                  (append (list '*) product-parts)))))))
